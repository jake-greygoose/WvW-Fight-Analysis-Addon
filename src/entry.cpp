...
void AddonLoad(AddonAPI* aApi) {
    ...
    for (auto& mainWindow : Settings::windowManager.mainWindows) {
        RegisterWindowForNexusEsc(mainWindow.get(), "WvW Fight Analysis");
    }

    for (auto& widgetWindow : Settings::windowManager.widgetWindows) {
        RegisterWindowForNexusEsc(widgetWindow.get(), "Team Ratio Bar");
    }

    if (Settings::windowManager.aggregateWindow) {
        RegisterWindowForNexusEsc(Settings::windowManager.aggregateWindow.get(), "Aggregate Stats");
    }
...
void AddonUnload() {
    ...
    for (auto& mainWindow : Settings::windowManager.mainWindows) {
        if (mainWindow->useNexusEscClose) {
            std::string identifier = "WvW Fight Analysis Main " + std::to_string(windowIndex);
