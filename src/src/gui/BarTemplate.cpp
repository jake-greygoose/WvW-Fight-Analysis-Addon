...
void BarTemplateRenderer::RenderTooltipStats(
    ...
    //ImGui::Text("Strips: %d", stats.totalStrips);
...
void BarTemplateRenderer::RenderTemplate(
    ...
    // Get the window bounds for clipping
    ImVec2 windowPos = ImGui::GetWindowPos();
    ImVec2 windowSize = ImGui::GetWindowSize();
    ImVec2 windowClipMin = windowPos;
    ImVec2 windowClipMax = ImVec2(windowPos.x + windowSize.x, windowPos.y + windowSize.y);

    float totalWidth = ImGui::GetContentRegionAvail().x;
    float totalHeight = ImGui::GetTextLineHeight() + 4.0f;

    bool isHovered = false;
    if (showTooltips && !ImGui::IsPopupOpen(NULL, ImGuiPopupFlags_AnyPopupId)) {  // Check if any popup is open
        ImVec2 mousePos = ImGui::GetMousePos();
        ImVec2 rectMin = startScreenPos;
        ImVec2 rectMax = ImVec2(startScreenPos.x + totalWidth, startScreenPos.y + totalHeight);

        // Check if mouse is within both the item area AND the window bounds
        bool inItemBounds = mousePos.x >= rectMin.x && mousePos.x < rectMax.x&&
            mousePos.y >= rectMin.y && mousePos.y < rectMax.y;

        bool inWindowBounds = mousePos.x >= windowClipMin.x && mousePos.x < windowClipMax.x&&
            mousePos.y >= windowClipMin.y && mousePos.y < windowClipMax.y;

        isHovered = inItemBounds && inWindowBounds;

        // Show tooltip if truly hovered
        if (isHovered) {
            ImGui::BeginTooltip();
