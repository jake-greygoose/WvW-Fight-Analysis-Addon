...
void BaseWindowSettings::updateDisplayName(const std::string& defaultName) {
    if (windowId.empty()) {
        fullDisplayName = defaultName + "##" + GenerateUniqueId("win_");
    }
    else if (!windowName.empty()) {
        fullDisplayName = windowName + "##" + windowId;
    }
    else {
        fullDisplayName = defaultName + "##" + windowId;
    }
}
...
AggregateWindowSettings::AggregateWindowSettings(const json& j) : BaseWindowSettings(j) {
    if (!j.is_null()) {
    ...
    void Settings::Load(std::filesystem::path aPath) {
        Settings::Mutex.lock();
        {
            try {
                ...
                if (!Settings.contains(IS_ADDON_AGG_WINDOW_VISIBLE)) {
                    Settings[IS_ADDON_AGG_WINDOW_VISIBLE] = false;
                }
                if (!Settings.contains(HIDE_AGG_WINDOW_WHEN_EMPTY)) {
                    Settings[HIDE_AGG_WINDOW_WHEN_EMPTY] = false;
                }
                ...
                try {
                    IsAddonAggWindowEnabled = Settings[IS_ADDON_AGG_WINDOW_VISIBLE].get<bool>();
                }
                catch (...) {
                    IsAddonAggWindowEnabled = false;
                    Settings[IS_ADDON_AGG_WINDOW_VISIBLE] = false;
                }

                try {
                    hideAggWhenEmpty = Settings[HIDE_AGG_WINDOW_WHEN_EMPTY].get<bool>();
                }
                catch (...) {
                ...
                if (windowManager.aggregateWindow) {
                    windowManager.aggregateWindow->isEnabled = IsAddonAggWindowEnabled;
                }
                ...
                Settings[IS_ADDON_AGG_WINDOW_VISIBLE] = IsAddonAggWindowEnabled;
                Settings[HIDE_AGG_WINDOW_WHEN_EMPTY] = hideAggWhenEmpty;
